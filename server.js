...
async function downloadAndCacheImage(imageUrl) {
  ...
  const ext =
    (imageUrl
      .split("?")[0]
      .match(/\.(jpg|jpeg|png|webp|gif|svg)$/i) || [".jpg"])[0] || ".jpg";
    ...
    const req = client.get(
      ...
      (response) => {
        ...
        if (response.statusCode !== 200) {
          ...
          fs.unlink(filepath, () => {});
          console.warn(
            `⚠  Image download failed (${response.statusCode}): ${imageUrl}`
          );
      ...
      const seedData = [
        {
          ...
          desc: "Classic Filipino adobo over steamed white rice",
          price: 75.0,
          cat: "Viand",
          img: "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=400&auto=format&fit=crop",
          stock: 20,
        },
        {
          name: "Sinigang na Baboy",
          desc: "Sour tamarind broth with tender pork ribs",
          price: 120.0,
          cat: "Soup",
        ...
        {
          name: "Pancit Canton",
          desc: "Stir-fried egg noodles with mixed vegetables",
          price: 85.0,
          cat: "Noodles",
          img: "https://images.unsplash.com/photo-1555126634-323283e090fa?w=400&auto=format&fit=crop",
          stock: 18,
        },
        {
          name: "Lechon Kawali",
          desc: "Deep-fried crispy pork belly with liver sauce",
          price: 110.0,
          cat: "Viand",
          img: "https://images.unsplash.com/photo-1544025162-d76694265947?w=400&auto=format&fit=crop",
          stock: 12,
        },
        {
          name: "Halo-Halo Special",
          desc: "Shaved ice dessert with mixed fruits and leche flan",
          price: 95.0,
          cat: "Dessert",
...
app.post("/products", async (req, res) => {
  ...
  if (
    image &&
    (image.startsWith("data:") || (!image.startsWith("http") && image.length > 300))
  ) {
    return res.status(400).json({
      error:
        "Hindi pwede ang base64 image. Gamitin ang image URL (http/https) galing sa internet.",
    });
  }
...
app.patch("/products/:id", async (req, res) => {
  ...
  if (image !== undefined) {
    fields.push("image = ?");
    values.push(resolvedImage || null);
  }
  if (stock !== undefined) {
    fields.push("stock = ?");
...
app.post("/cart", async (req, res) => {
  ...
  try {
    const [prod] = await pool.execute("SELECT id FROM products WHERE id = ?", [
      productId,
    ]);
    ...
    if (existing.length) {
      ...
      const [u] = await pool.execute("SELECT * FROM cart WHERE id = ?", [
        existing[0].id,
      ]);
      row = u[0];
    } else {
      ...
      const [i] = await pool.execute("SELECT * FROM cart WHERE id = ?", [
        result.insertId,
      ]);
      row = i[0];
    }
...
app.delete("/cart/:id", async (req, res) => {
  try {
    const [existing] = await pool.execute("SELECT id FROM cart WHERE id = ?", [
      req.params.id,
    ]);
    if (!existing.length)
...
app.patch("/orders/:id", async (req, res) => {
  ...
  try {
    const [existing] = await pool.execute("SELECT id FROM orders WHERE id = ?", [
      req.params.id,
    ]);
...
app.post("/admin/reseed-images", async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (localUrl !== img) {
        await pool.execute("UPDATE products SET image = ? WHERE id = ?", [
          localUrl,
          row.id,
        ]);
        updated++;
